...
public class Practice {
    ...
    public static List<Employee> getAllEmployees() {
       return new ArrayList<>(employeeService.readAll());
             //employeeService.readAll().stream().collect(Collectors.toList());
    }


    // Display all the countries
    public static List<Country> getAllCountries() {
        return new ArrayList<>(countryService.readAll());
    }

    // Display all the departments
    public static List<Department> getAllDepartments() {
        return new ArrayList<>(departmentService.readAll());
    }

    // Display all the jobs
    public static List<Job> getAllJobs() {

        return new ArrayList<>(jobService.readAll());
    }

    // Display all the locations
    public static List<Location> getAllLocations() {

        return new ArrayList<>(locationService.readAll());
    }

    // Display all the regions
    public static List<Region> getAllRegions() {

        return new ArrayList<>(regionService.readAll());
    }

    // Display all the job histories
    public static List<JobHistory> getAllJobHistories() {

        return new ArrayList<>(jobHistoryService.readAll());
    }
...
//      1.way         .allMatch(employee -> employee.getSalary()>1000);

                .noneMatch(employee -> employee.getSalary()<1000); //2. way

//    3. way    to make it faster performance
//        return !employeeService.readAll().stream()
//               .anyMatch(employee -> employee.getSalary()<1000);
    ...
    public static Long getMaxSalary() throws Exception {
        return employeeService.readAll().stream()
                ...
                .reduce((t1,t2)-> t1>t2 ? t1 : t2)  //.reduce(Long::max)
                .get();

        // 2 way : max(Comparator.comparing(Employee::getSalary)).get().getSalary();
        // 3 way : sorted(Comparator.comparing(Employee::getSalary).reversed()).findFirst().getSalary();
    }
    ...
    public static List<Employee> getMaxSalaryEmployee()  {
        return employeeService.readAll().stream()
                .filter(employee -> {
                    try {
                        return Objects.equals(employee.getSalary(), getMaxSalary());
                    } catch (Exception e) {
    ...
    public static Long getMaxSalaryInAmericasRegion() throws Exception {
        return employeeService.readAll().stream()
                ...
                .orElseThrow(()->new RuntimeException("NO Employee")).getSalary();
    ...
    public static List<Employee> getAllEmployeesDepartmentIdIs90or60or100or120or130() {
        return employeeService.readAll().stream()
                .filter(employee -> employee.getDepartment().getId() ==90 || employee.getDepartment().getId() ==60
                        || employee.getDepartment().getId() ==100|| employee.getDepartment().getId() ==120
